...
public class IntercepTestGenerator : ISourceGenerator
{
    ...
    private static void GenerateFunction(LocalFunctionStatementSyntax candidateMethod,
        IImmutableSet<INamedTypeSymbol> namedSymbolsForAssembly, IntercepTestMockAttributeSyntaxReceiver receiver,
        Dictionary<string, MethodDeclarationSyntax> generatedMethods)
    {
        var attribute = candidateMethod.AttributeLists.Select(al => al.Attributes.First(a =>
            ((IdentifierNameSyntax)a.Name).Identifier.ValueText == "IntercepTestMockAttribute" ||
            ((IdentifierNameSyntax)a.Name).Identifier.ValueText == "IntercepTestMock")).First();


        //todo clean up
        var typeToMockExpression =
            (IdentifierNameSyntax)((TypeOfExpressionSyntax)attribute.ArgumentList.Arguments[0].Expression).Type;
        var functionToMockExpression =
            (MemberAccessExpressionSyntax)((InvocationExpressionSyntax)attribute.ArgumentList.Arguments[1].Expression)
            .ArgumentList.Arguments[0].Expression;
        var typeToMock = namedSymbolsForAssembly.First(t => t.Name == typeToMockExpression.Identifier.ValueText);


        var callingTypeExpression =
            (IdentifierNameSyntax)((TypeOfExpressionSyntax)attribute.ArgumentList.Arguments[2].Expression).Type;
        var callingFunctionExpression =
            (MemberAccessExpressionSyntax)((InvocationExpressionSyntax)attribute.ArgumentList.Arguments[3].Expression)
            .ArgumentList.Arguments[0].Expression;
        var callingType = namedSymbolsForAssembly.First(t => t.Name == callingTypeExpression.Identifier.ValueText);
        var callingFunction = callingType.GetMembers().OfType<IMethodSymbol>().FirstOrDefault(m =>
            m.Name == ((IdentifierNameSyntax)callingFunctionExpression.Name).Identifier.ValueText);

        var location = receiver.MemberAccessExpressions[
            new MethodAccessExpressionKey(callingTypeExpression.Identifier.ValueText,
                ((IdentifierNameSyntax)callingFunctionExpression.Name).Identifier.ValueText,
                typeToMockExpression.Identifier.ValueText,
                ((IdentifierNameSyntax)functionToMockExpression.Name).Identifier.ValueText)].GetLineSpan();
        ...
        var parameters = new List<ParameterSyntax>
        {
            SyntaxFactory.Parameter(SyntaxFactory.Identifier("p"))
                ...
                .WithType(SyntaxFactory.ParseTypeName(typeToMock.ToDisplayString(
        ...
        var generatedMethodName = typeToMockExpression.Identifier.ValueText +
                                  ((IdentifierNameSyntax)functionToMockExpression.Name).Identifier.ValueText;
        MethodDeclarationSyntax methodDeclaration;
        if (generatedMethods.ContainsKey(generatedMethodName))
        {
            methodDeclaration = generatedMethods[generatedMethodName];

            var ifSyntax = IfStatementSyntax(testClass.Identifier.ValueText, testMethod.Identifier.ValueText,
                candidateMethod);
            methodDeclaration = methodDeclaration.AddBodyStatements(ifSyntax);
            methodDeclaration.AttributeLists.Add(SyntaxFactory.AttributeList(SyntaxFactory.SeparatedList(
                    ...
                    SyntaxFactory.Attribute(SyntaxFactory.ParseName("InterceptsLocation"),
                        SyntaxFactory.AttributeArgumentList(SyntaxFactory.SeparatedList(new List<AttributeArgumentSyntax>
                        {
                            SyntaxFactory.AttributeArgument(SyntaxFactory.LiteralExpression(
                                ...
                                SyntaxFactory.Literal(callingFunction.DeclaringSyntaxReferences[0].SyntaxTree.FilePath))),
                            SyntaxFactory.AttributeArgument(SyntaxFactory.LiteralExpression(
                                SyntaxKind.NumericLiteralExpression,
                                SyntaxFactory.Literal(location.StartLinePosition.Line + 1))),
                            SyntaxFactory.AttributeArgument(SyntaxFactory.LiteralExpression(
                                SyntaxKind.NumericLiteralExpression,
                                SyntaxFactory.Literal(location.StartLinePosition.Character + 1)))
                        })))
                })));
